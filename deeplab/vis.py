...
from PIL import Image #pang-add
...
# The format to save image.  #pang-add
_IMAGE_FORMAT_LABEL = '%06d_image_groundtruth'
...
def main(unused_argv):
  ...
  with g.as_default():
    ...
    #labels = tf.reshape(samples[common.LABEL], shape=[-1]) #pang-add
    ...
    while (FLAGS.max_number_of_iterations <= 0 or
      ...
      tf_config = tf.ConfigProto() #pang-add  #set-GPU-limitaion
      tf_config.gpu_options.allow_growth = True #pang-add
      with sv.managed_session(FLAGS.master,
                              start_standard_services=False,config=tf_config) as sess: #pang-add
        '''
        tf_config = tf.ConfigProto() #pang-add
        tf_config.gpu_options.allow_growth = True #pang-add
        sess = tf.Session(config=tf_config) #pang-add
        '''
        ...
        #for batch in range(3):#pang-add
          tf.logging.info('Visualizing batch %d / %d', batch + 1, num_batches)
          _process_batch(sess=sess,
                         original_images=samples[common.ORIGINAL_IMAGE],
                         labels=samples[common.LABEL], #pang-add
                         mybatch_num=batch + 1, #pang-add
      ...
      tf.logging.info(
          'Finished visualization at ' + time.strftime('%Y-%m-%d-%H:%M:%S',
                                                       time.gmtime()))
      print('111111111111111111111111') #pang-add
      end = time.time()
      print(end-start)
      print('meicidiedaihaoshi:')
      print((end-start)/num_batches*1.0)
      print('s/img---------------------:')
      print(((end-start)/num_batches*1.0)/float(FLAGS.vis_batch_size))
